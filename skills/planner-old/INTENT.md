...
```
python3 -m skills.planner_old.cli.qr --state-dir <dir> --qr-phase <phase> update-item <id> --status <status> [--finding <text>]
...
python3 -m skills.planner_old.cli.qr --state-dir /tmp/state --qr-phase plan-design \
...
```
python3 -m skills.planner_old.cli.plan --state-dir <dir> set-intent \
...
<parallel_dispatch agent="quality-reviewer" count="N">
  ...
  <template>
    <invoke cmd="python3 -m skills.planner_old.quality_reviewer.{phase}_qr_verify --step 1 --state-dir {state_dir} --qr-items $GROUP_ITEMS" />
  </template>
</parallel_dispatch>
```
...
```
python3 -m skills.planner_old.quality_reviewer.<phase>_qr_decompose --step 1 --state-dir {state_dir}
```
...
```
python3 -m skills.planner_old.quality_reviewer.<phase>_qr_verify --step 1 --state-dir {state_dir} --qr-items qa-001,qa-002
```
...
```
State Mutation:
  python3 -m skills.planner_old.cli.plan --state-dir {state_dir} set-intent \
