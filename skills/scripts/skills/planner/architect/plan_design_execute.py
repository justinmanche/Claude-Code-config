    ...
    if step == 1:
        ...
        return {
            ...
            "actions": [
                ...
                "  - context.json verification_env (unit, integration, ship, live)",
                "  - User conversation hints",
                "  - Project CLAUDE.md / README.md",
                "  - conventions/structural.md domain='testing-strategy'",
                "",
                "DECIDE PER MILESTONE which layers must prove it:",
                "  - Touches SQL, schema, access policies, tenant/ownership scope, or a",
                "    write/read on behalf of another party -> integration_tests against",
                "    the REAL dependency, run as the identity that will run it in",
                "    production (incl. the counterparty for cross-tenant flows).",
                "    Mocked unit tests cannot see wrong columns, casts or policy denials.",
                "  - Changes anything a user sees or does -> live_checks: observable,",
                "    role-specific steps on the deployed system ('as <role>, do X, see Y').",
                "",
        ...
        return {
            ...
            "actions": [
                ...
                "    set-verification --milestone M-001 --integration-test '<test>' --live-check '<check>'",
                "      (flags repeatable; in batch mode pass JSON lists:",
                "       {\"integration_tests\": [...], \"live_checks\": [...]})",
                "",
                ...
                "  - Code Intent: WHAT to change, precise enough to implement directly.",
                "    There is NO later planned-diff phase: developers implement from",
                "    intents, and the reviewer checks each intent against the real code.",
                "    Name the exact table/column/enum/permission/function involved.",
                "  - Tests: unit test scenarios",
                "  - integration_tests / live_checks: per the step 3 decision (set-verification)",
                "",
                "PARALLELIZATION:",
                ...
                f"  python3 -m skills.planner.cli.plan --state-dir {kwargs.get('state_dir', '$STATE_DIR')} validate --phase plan-design",
                "",
                "REFERENCE SCHEMA:",
                "",
                plan_json_schema,
