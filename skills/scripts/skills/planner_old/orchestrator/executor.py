...
from skills.planner_old.shared.qr.utils import (
    qr_file_exists,
...
from skills.planner_old.shared.resources import get_mode_script_path
...
from skills.planner_old.shared.constants import (
...
MODULE_PATH = "skills.planner_old.orchestrator.executor"
...
def step_init(ctx: dict) -> dict:
    ...
    if args.reconcile and pending:
        ...
        command = "python3 -m skills.planner_old.quality_reviewer.exec_reconcile --step 1 --milestone $number"
...
def step_impl_code_work(ctx: dict) -> dict:
    ...
    plan = _load_plan(state_dir)
    state = _load_exec_state(state_dir)
    invoke_cmd = f"python3 -m skills.planner_old.developer.exec_implement --step 1 --state-dir {state_dir}"
...
def step_impl_docs_work(ctx: dict) -> dict:
    ...
    qr = ctx["qr"]
    state = _load_exec_state(state_dir)
    invoke_cmd = f"python3 -m skills.planner_old.technical_writer.exec_docs --step 1 --state-dir {state_dir}"
