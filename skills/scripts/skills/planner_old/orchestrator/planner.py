...
from skills.planner_old.shared.qr.types import QRState, QRStatus, LoopState
from skills.planner_old.shared.gates import build_gate_output, GateResult
from skills.planner_old.shared.qr.cli import add_qr_args
from skills.planner_old.shared.qr.utils import qr_file_exists, increment_qr_iteration
from skills.planner_old.shared.resources import get_mode_script_path, PlannerResourceProvider
from skills.planner_old.shared.builders import THINKING_EFFICIENCY, format_forbidden
from skills.planner_old.shared.constraints import (
    ORCHESTRATOR_CONSTRAINT_EXTENDED,
    format_state_banner,
)


MODULE_PATH = "skills.planner_old.orchestrator.planner"
...
def _translate_plan(state_dir: str) -> str | None:
    ...
    from skills.planner_old.cli.plan_commands import PlanContext, _translate
...
def verify_step(title, actions):
    ...
    def handler(ctx):
        from skills.planner_old.shared.resources import validate_state_dir_requirement
...
def qr_verify_step(title, phase):
    ...
    """
    def handler(ctx):
        from skills.planner_old.shared.qr.utils import load_qr_state, query_items, by_status, by_blocking_severity
        from skills.planner_old.shared.qr.phases import get_phase_config
...
def get_step_guidance(step: int, qr_status, state_dir) -> dict | str:
    ...
    """
    from skills.planner_old.shared.qr.utils import get_qr_iteration, has_qr_failures
...
def main():
    ...
    from skills.planner_old.shared.constants import PLANNER_GATE_STEPS, PLANNER_TOTAL_STEPS
    ...
    if args.step > 1 and args.state_dir:
        from skills.planner_old.shared.schema import validate_state, SchemaValidationError
        try:
    ...
    if isinstance(result, GateResult):
        ...
        if result.terminal_pass and args.state_dir:
            ...
            if plan_path:
                ...
                print(f"  python3 -m skills.planner_old.orchestrator.executor --step 1 --state-dir {args.state_dir}")
    else:
        print(result)
