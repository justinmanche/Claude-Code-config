...
from skills.planner_old.shared.qr.utils import load_qr_state
...
def format_assign_cmd(state_dir: str, phase: str, prefix: str) -> str:
    ...
    """
    return f"""OUTPUT:
  python3 -m skills.planner_old.cli.qr --state-dir {state_dir} --qr-phase {phase} \\
...
def step_9_structural_grouping(state_dir: str, phase: str, module_path: str) -> dict:
    ...
    return {
        ...
        "actions": [
            ...
            "",
            f"Execute via CLI:",
            f"  python3 -m skills.planner_old.cli.qr --state-dir {state_dir} --qr-phase {phase} \\",
