...
from skills.planner_old.shared.constraints import format_state_banner
from skills.lib.conventions import get_convention
from skills.planner_old.shared.resources import (
    STATE_DIR_ARG_REQUIRED,
    get_context_path,
    render_context_file,
    validate_state_dir_requirement,
)
from skills.planner_old.shared.temporal_detection import format_as_prose, format_actions
...
def get_step_guidance(
    ...
    MODULE_PATH = module_path or "skills.planner_old.technical_writer.plan_docs_execute"
    ...
    if step == 1:
        ...
        actions.extend([
            ...
            "  python3 -m skills.planner_old.cli.plan --state-dir $STATE_DIR set-doc-diff \\",
            "    --change CC-M-001-001 --version 1 --content-file /tmp/doc.diff",
            "",
            "  # Create documentation-only change (README, etc.):",
            "  python3 -m skills.planner_old.cli.plan --state-dir $STATE_DIR create-doc-change \\",
            "    --milestone M-001 --file path/README.md --content-file /tmp/readme.diff",
            "",
            "BATCH MODE:",
            "  python3 -m skills.planner_old.cli.plan --state-dir $STATE_DIR batch '[",
        ...
        return {
            ...
            "actions": [
                ...
                "  python3 -m skills.planner_old.cli.plan list-decisions",
                "",
        ...
        return {
            ...
            "actions": [
                ...
                "",
                "Run validation:",
                f"  python3 -m skills.planner_old.cli.plan validate --phase plan-docs --state-dir {state_dir}",
                "",
